...
from .forms import CheckoutForm, PaymentForm, CouponForm, RefoundForm, PaymentForm
                ...
                if use_default_shipping:
                    ...
                    if is_valid_form([shipping_address1, shipping_country, shipping_city, shipping_zip]):
                        shipping_address = Address(
                            user=self.request.user,
                            streat_address=shipping_address1,
                ...
                if same_billing_address:
                    ...
                    if is_valid_form([billing_address1, billing_country, billing_city, billing_zip]):
                        billing_address = Address(
                            user=self.request.user,
                            streat_address=billing_address1,
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if save:
                if userprofile.stripe_customer_id != '' and userprofile.stripe_customer_id is not None:
                    customer = stripe.Customer.retrieve(
                        userprofile.stripe_customer_id)
                    customer.sources.create(sources=token)
                else:
            ...
            try:
                ...
                # Assing the payment tp an order
                ...
                messages.success(
                    self.request, 'Congrats, Your order was successfully')
                return redirect('/')
                ...
                messages.warning(
                    self.request, 'Too many requests, rate limit error.')
                return redirect('/')
                ...
                # Something else happened, send an email to ourselves
                messages.warning(
                    self.request, 'A serious error, we have been notified.')
                return redirect('/')
...
class RequestRefoundView(View):
    def get(self, *args, **kwargs):
        form = RefoundForm()
        context = {
            'form': form
        }
        return render(self.request, 'request_refound.html', context)
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                refound = Refound()
                refound.order = order
                refound.resson = message
                refound.email = email
                refound.save()

                messages.success(
                    self.request, "Your request was received.")
                return redirect('core:request-refound')

            except ObjectDoesNotExist:
                messages.info(
                    self.request, "This order was not exist.")
                return redirect('core:request-refound')
